...
the Monte-Carlo method with focei.
...
the variances/covariance of the residual errors.
...
- `mceta=-1` -- Use the best eta estimate from the last iteration
  (`nlmixr2`'s default, and not available in NONMEM at the time of this writing)
...
- `mceta=N` -- Try the `etas` as described in `mceta=1` and then take
  `N-1` random multivariate normal samples (assuming `omega` is
  true). Start the optimization at the `eta` value with the smallest
  individual objective function.

This makes the `focei` algorithm a Monte-Carlo method (though not a
`mcmc` method).

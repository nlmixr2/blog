...
mixed effects models is to choose reasonable initial estimates for the
...
to get right (depending on the data, model and algorithm).
...
- `mceta=1` -- Try the `eta` at the last best fit estimate as well as
  the `eta`s set to zero, and start the optimization at the `eta` with

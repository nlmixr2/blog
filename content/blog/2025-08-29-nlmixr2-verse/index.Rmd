...
longer possible (you have to call the corresponding package where the
...
With nlmixr2 4.0, a new function `nlmixr2fix()` has been introduced.
This function tries to import an old saved model in the current
nlmixr2 ecosystem.  Often this can be done by re-parsing the original
model because of language changes.  This is what needed to be done to
convert a `nlmixr2` 3.0 model to a `nlmixr2` 4.0 model.

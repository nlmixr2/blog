...
There are two gradient-free optimization methods.
...
- `optim` that uses the R function `stats::optim` for the gradient-free
  ...
  `optimControl(method="BFGS")` and `optimControl(method="SANN")` and
...
- At the solution the table of individual predictions, residuals etc
  are automatically to an output table for quick diagnostic plots.
...
- `lbfgsb3c` to estimate population only

...
'rxode2' 2.0.9 has been released today! I want to personally thank all
those who have submitted issues, and helped with the development.
Without the support 'rxode2' wouldn't be the tool it is today.

This is the CRAN visible release since 'rxode2' 2.0.7 and I
would like to highlight a few new interesting features:

## 'rxode2' can how have models functions that are more flexible
...
## Why is 'rxode2' breaking 'nlmixr2'

CRAN had requested that we reduce the compile time for 'rxode2' to
keep remain on CRAN.  This effectively was requesting the compile be
split out to separate packages. With the restructuring, 'nlmixr2'
needed to be restructured too.  We are actively resubmitting a
compatible 'nlmixr2' now. In the mean time you can use the development
versions with r-universe:

``` r
install.packages(c("rxode2ll", "rxode2parse", "rxode2random",
                   "rxode2et", "rxode2",
...
- 'rxode2et' which is the split off of the 'rxode2' event table
  function 'et()'

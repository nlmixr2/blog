...
// --draft: use the free piper clips from `tts_generate.py --draft`
const DRAFT = process.argv.includes('--draft');
const AUDIO_DIR = DRAFT ? 'audio-draft' : 'audio';
const AUDIO_EXT = DRAFT ? '.wav' : '.mp3';
const BUILD = 'build';
...
(async () => {
  ...
  const OUT = DECK.replace(/\.html?$/i, '') + (DRAFT ? '-draft.mp4' : '.mp4');

...
function videoChunk(video, mp3, secs, out, overlayPng) {
  const args = ['-y', '-loglevel', 'error', '-i', video];
  if (overlayPng) args.push('-i', overlayPng);

  let afilter, amap;
  if (mp3) {
    args.push('-i', mp3);   // after the overlay, so its stream index shifts
    const ai = overlayPng ? 2 : 1;
    afilter = `[${ai}:a]adelay=${Math.round(LEAD_S * 1000)}:all=1,apad[a]`;
    amap = '[a]';
  ...
  // A narrated background-video slide still has reveal's own text on it
  // (a title, a heading).  The chunk is spliced from the FILE, so that text
  // is only in the render if it is composited back on as a transparent
  // overlay captured from the page.
  const vchain = `[0:v]${VFIT},tpad=stop_mode=clone:stop_duration=${secs.toFixed(3)}`;
  const filter = (overlayPng
                    ? `${vchain}[bg];[bg][1:v]overlay=0:0:format=auto[v]`
                    : `${vchain}[v]`)
               + (afilter ? `;${afilter}` : '');
  ...
  try {
    ...
    for (const { h, id, video, videoMuted } of slides) {
      ...
      if (video) {
        ...
        // Capture the slide's own text with everything else transparent, so
        // it can sit on top of the film.  Only worth doing when the slide is
        // narrated: an unmuted sting is the film alone.
        let overlay = null;
        if (narrate) {
          overlay = path.join(BUILD, `overlay${h}.png`);
          await page.evaluate(i => Reveal.slide(i, 0), h);
          await new Promise(r => setTimeout(r, 450));
          const hide = await page.addStyleTag({ content:
            '.reveal-viewport,.reveal,.slides,.slides section{background:transparent !important}'
            + '.slide-background,.slide-background-content{display:none !important}' });
          await page.screenshot({ path: path.join(root, overlay), type: 'png',
                                  omitBackground: true });
        ...
        videoChunk(src, narrate ? path.join(root, mp3) : null, chunkSecs,
                   path.join(root, chunk), overlay && path.join(root, overlay));

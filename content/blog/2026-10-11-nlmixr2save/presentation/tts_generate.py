...
DECK   = pathlib.Path(__file__).with_name("nlmixr2save.html")
...
# no apt/pip needed.  https://github.com/rhasspy/piper/releases
...
# Energy.  A take at stability 0.35 / style 0.55 / speed 1.0 came back sounding
# tired on this deck, so the read is lifted rather than sped up: LOWER stability
# lets the delivery vary more, HIGHER style is where the warmth and emphasis
# live, and speed moves only a little -- 1.13 is the pronunciation ceiling
# (see below) and "rushed and nervous" was the complaint on an earlier deck, so
# the lift has to come from expression, not pace.
...
SAY = [
    (r"\bphi0\b",            "phi zero"),
    (r"(?i)\bfocei\b",       "F-O-C-E-I"),   # covers FOCEI/focei/FOCEi
    (r"(?i)\bfoce\b",        "F-O-C-E"),     # FOCE on its own, and "FOCE P"
    (r"\bSAEM\b",            "S-A-E-M"),
    (r"\bsaem\b",            "S-A-E-M"),
    (r"\bBOBYQA\b",          "B-O-B-Y-Q-A"),
    (r"\bbobyqa\b",          "B-O-B-Y-Q-A"),
    (r"\bnlmixr2save\b",     "N L mixer two save"),
    (r"\bnlmixr2est\b",      "N L mixer two E-S-T"),
    (r"\bnlmixr2\b",         "N L mixer two"),
    (r"\bACoP\b",            "A-cop"),
    (r"\brxSolve\b",         "R-X solve"),
    (r"\brxode2\b",          "RXODE2"),
...
def slides_from(deck: pathlib.Path):
    ...
    for i, s in enumerate(secs):
        ...
        if m:                                   # normal slide
            ...
            # ...and no "?." / "!." either: a note ending in a question mark
            # still gets ". " appended by the </p> rule above, and the model
            # audibly stumbles on the doubled terminator.
            t = re.sub(r"([?!])\s*\.", r"\1", t)
        ...
        if not t:
            # A slide with no notes is nearly always a mistake, so this aborts
            # -- except for a background-video slide, where it is the way to
            # say "let the film's own audio play".  render_presentation.js
            # already builds that chunk from the file when no clip exists, so
            # the clip has to be absent rather than silent.
            if "data-background-video" in s.split(">", 1)[0]:
                print(f"slide {i} ({sid}): background video with no notes "
                      f"-- no clip, it keeps its own audio")
                continue
...
def draft(slides):
    ...
    that splits one sentence into two, an un-normalised acronym, a garbled
    camelCase token.  It will not tell you whether ElevenLabs pronounces
    something correctly -- piper's phonemizer is different -- but it WILL
    tell you whether the text has an unwanted gap, a run-on, or an odd
    sentence break, which is most of what a first listen is for.
    """
    if not PIPER_BIN.exists():
        sys.exit(f"no piper binary at {PIPER_BIN} -- download a release from "
                  "github.com/rhasspy/piper (self-contained tarball, no "
                  "apt/pip needed) and a voice .onnx from "

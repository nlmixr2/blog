...
<iframe src="https://youtube.com/embed/Jf9QbmuR_Xw"
        title="Model diagrams and equations, straight from the code"
...
[Watch on YouTube](https://youtu.be/Jf9QbmuR_Xw)
...
templates, you get the diagram and the equations without doing
anything.  From version 0.2.3 (currently the development version, on
its way to CRAN) the default report opens with the model: the diagram
from `modelDiagram()`, then the equations as native, editable Word and
PowerPoint math, ahead of the parameter table and the goodness-of-fit
figures `nlmixr2rpt` already produces.  Until it reaches CRAN, install
it with `remotes::install_github("nlmixr2/nlmixr2rpt")`.

Reporting a fit is the same call as before:

```{r rpt-run, eval = FALSE, code = local({x <- readLines("nlmixr2rpt/make-report.R"); x[grep("^## Report the fit", x):length(x)]})}
```

These are the first two pages of the Word report:

![The first two pages of the nlmixr2rpt Word report: the model diagram under a "Model Structure" heading, then the model equations as Table 1.](nlmixr2rpt/nlmixr2rpt-pages.png)

The equations come from a new function, `gen_eqn_table(fit, obnd)`,
which puts one equation per row of a `flextable` and writes each row
as native math.  With one equation per row there are no alignment
points, so Word and LibreOffice show the Word report the same way.  If
you keep your own report configuration, add the diagram and the
equations to it with the same two entries the default now uses:
...
tables:
  model_equations:
    cmd: |-
      t_res <- list(ft = list(gen_eqn_table(fit, obnd)))
```

In PowerPoint the equations slide gets the same native math.  There is
no fallback image, though, so LibreOffice Impress and some other
viewers show that slide blank.
...
`knit_print()` equations are in `nlmixr2extra`; and `nlmixr2rpt` 0.2.3
puts both in its Word and PowerPoint reports by default.  Point them at the

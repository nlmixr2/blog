...
::: {.notes}
If your reports already come from the nlmixr2 report package, the
diagram and the equations are now built in.
:::

## nlmixr2rpt 0.2.3 opens every report with the model diagram and equations

:::: {.columns .codemid}
::: {.column width="44%"}
```r
obnd <- report_fit(fit = fit,
                   obnd = obnd)
save_report(obnd, "fit-report.docx")
```

::: rule
:::

- No configuration changes: the default report includes both
- `gen_eqn_table()`: native, editable equations in Word and PowerPoint
- Until it reaches CRAN, install it from GitHub
:::

::: {.column width="56%"}
![](images/fig-rpt-pages.png){.nostretch .plot fig-alt="The first two pages of the nlmixr2rpt Word report: the model diagram under a Model Structure heading, then the model equations as Table 1"}
:::
::::

::: {.notes}
The nlmixr2 report package builds Word and PowerPoint reports from a
fit, using your organization's templates. In its next release, the
default report opens with the model: the diagram, then the equations,
ahead of the parameter table and the goodness of fit figures it
already makes.

So reporting a fit is the same call as before, with no changes to the
configuration file. The equations come from a new function that puts
one equation per row of a table, and writes each row as a native,
editable equation, in Word and in PowerPoint.
...
- Word and PowerPoint reports: [nlmixr2rpt]{.nlmixr} 0.2.3 includes both by default
...
And the nlmixr2 report package now puts both in its Word and PowerPoint
reports by default.

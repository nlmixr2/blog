...
Second, how nlmixr2 plot draws a compartment diagram straight from the
model.
...
And fifth, adding the diagram and the equations to nlmixr2 R P T
reports.
:::
...
from the control stream. Then the model changes. A transit compartment
...
reads the d d t equations, works out which terms move mass between
compartments, and then places the compartments the way a
pharmacometrician would draw them by hand. The dosing compartment is
on top with a heavy border, the peripheral compartment is to the left,
and elimination leaves the central compartment going down.

With labels set to true, each arrow also carries the model term that
drives it. That's usually what you want in a methods section.
:::
...
::: {.notes}
How does it know? Every d d t equation is split into its terms, and
each term is classified by what it does to mass.

Here's the central compartment. k a times depot is also subtracted
from depot, so it's a transfer in from depot. c l over v times central
contains central's own amount, and nothing else gains it, so it's an
elimination. q over v times central is subtracted here and added to the
peripheral compartment, so it's a transfer out. And the last term is
the transfer back in from the peripheral compartment.

There are two more kinds. A gain that depends on no other compartment,
like a zero order k in, is an input. And a term that depends on
another compartment without moving any mass, like an effect
compartment or a drug effect on turnover, is an interaction. It's
drawn as a dashed or dotted arrow.
Intermediate variables are followed, so it doesn't matter if you
compute a concentration first.
:::
...
::: {.notes}
Pharmacodynamic models are where hand-drawn diagrams get fiddly, and
where this saves the most time.

This is an indirect response model where the drug inhibits production
of the response. Nobody told model diagram that effect is a turnover
response, that gut is a transit compartment, or that the drug is
inhibitory.

It followed P D back through the concentration to the center
compartment, saw that P D falls as the concentration rises, and drew
an inhibition, the dotted arrow. If you later change the model to a
stimulatory effect, the diagram changes with it.
:::
...
::: {.notes}
Target mediated drug disposition works the same way. The binding term,
k on times central times target, is lost from both the drug and the
target, and gained by the complex. So both partners get an arrow into
the complex, and unbinding gets arrows back out again.
...
The g g plot two engine gives you an ordinary g g plot object. You can theme it to
...
And dot gives you the Graphviz source as text. When the automatic
layout is ninety five percent of the way there, that's your escape
hatch. Edit the dot, instead of starting a drawing from scratch.

And if you want to see how the equations were read, model graph gives
...
::: {.notes}
nlmixr2 extra has a knit print method for model objects and for fits.
...
## Covariate switches survive the translation, and so does the error model
...
$$
\begin{aligned}
\mathrm{if} & \left({SEX}{\equiv}{1}\right) \{ \\
...
covariate switches. Here, the clearance depends on sex, and the if and
else branches come through as they're written. The double equals shows
up as an equivalence sign.

One thing to keep in mind is that the variable names are the symbols.
The name eta dot c l stays eta dot c l. It isn't turned into a Greek letter. So
...
If the final model changes during review, you re-knit, and all three
update. The report can't describe one model while its tables report
...
For the diagram, use the g g plot two engine, so it's a static image
...
::: {.notes}
If your reports already come from nlmixr2 R P T, the diagram and the
...
::: {.notes}
nlmixr2 R P T builds Word and PowerPoint reports from a fit, using your
organization's templates. It reads its figures and tables from a YAML
file, and each one is just R code that has the fit available.
...
and the goodness of fit figures that nlmixr2 R P T already makes.

On the right are the first two pages of the Word report it produced.
:::

## One equation per table row becomes native, editable Word math
...
- `as_equation()` writes **native, editable** Word math
- One equation per row: no alignment points, so Word and LibreOffice agree
- PowerPoint gets Office Math, with no fallback for other viewers
...
::: {.notes}
The equations table uses a small helper, defined once in the YAML's
table preamble.
...
each row as a native Word equation, so it can be edited like any other
equation.
...
PowerPoint deck, the same table is written as Office math, which
PowerPoint reads, but other viewers may show that slide blank.

The one extra requirement is the equa tags package. Without it, the
...
And for Word and PowerPoint reports, add two entries to your nlmixr2
R P T YAML.

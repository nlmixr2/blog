...
::: {.notes}
The first piece of work -- Bill, the AI agents and me -- was setting things up
so each individual can be solved and optimized independently.  Once it was on
CRAN we found OpenMP crashed R on Windows; Bill fixed that.
:::
...
::: {.notes}
Both lanes still find the individual MAP estimates with a gradient-based method
using exact gradients.  It is only the population problem that changes.
:::
...
::: {.notes}
This is the change that surprised me.  Recalculating the Hessian every outer
step costs something, and it still came out ahead on both the answer and the
clock, so it is on by default.
:::
...
::: {.notes}
`nlminb` split its work between calculating gradients and evaluating points,
which is why we chose it originally.  It can still give a "false convergence"
message that I have often found misleading.
:::
...
::: {.notes}
It does cost some runtime, because the objective re-solves the ODE.  The
inner optimization also moved from Nelder-Mead to `newuoa`; the old behavior
is `saemControl(type="nelder-mead", nonMuTheta="eta")`.
:::
...
::: {.notes}
I did not change the generator for its own sake.  The dashed box is a future
release, not this one -- but it only works if the chain state at the end of the
SA phase is something you can pick back up.
:::
...
::: {.notes}
Alongside this there is a large suite of new tableaus, Rosenbrock and implicit
Runge-Kutta methods, SUNDIALS CVODE and LSODE.  There is an "ODE solvers"
article in rxode2 covering all of them.
:::

...
2. fast exact population gradients
3. Changes to the defaults.
...
::: {.notes}
First, Bill, Hidde, and I worked on parallelizing the estimation.
:::
...
::: {.notes}
In the old releases every subject waited its turn.  Now, they
all solve and optimize together for the conditional estimation
methods and most of our other
...
every individual to get the best individual estimates.  Then,
...
::: {.notes}
The gradient dictates the optimizer you want to use.  The standard method
...
calculated, no finite differences are ever used with it.  However,
...
::: {.notes}
Because the fast method starts its individual search by projecting where the individual estimate
should be for the next optimization, it takes fewer steps to find the right
individual estimate at a new theta.
:::
...
to zig-zag to the minimum.  Now, by recalculating the curvature
...
::: {.notes}
Because of these updates, we have new defaults in the standard method.
:::
...
::: {.notes}
Our standard method now optimizes using a quadratic curve through
spread-out samples. This naturally follows the minimization trend
instead of getting stuck in the noise.  This is especially useful for
hard or complex surfaces.
:::
...
on them were continuously sampled until the variance shrank at the
end.  While this usually settled, it could become biased if it didn't
settle exactly where it needed to.  If we use a bounded
regression -- it honors the exact location and drops the error
measured by RMSE by 16-fold.
:::
...
to safely extend the chain rather than throwing away previous fits.
:::
...
::: {.notes}
Finally, we've added more experimental ODE solvers and Nonlinear mixed effect estimation methods
:::
...
there are many new ODE solving methods available in our packages.
:::
...
::: {.notes}
We've also introduced new experimental optimization methods like IMP, VAE and
new conditional-estimation variants.
:::
...
::: {.notes}
To summarize: parallel solving works on every OpenMP-enabled
platform, the fast methods utilizes an exact population gradient, our
classic method defaults have moved to a derivative-free optimizer
for better accuracy on
noisy parameters, SAEM is more robust, and we have many new
experimental ODE and estimation routines.
:::
